...
export class RedmineAPI {
	...
	static async getTimeEntries(issueId: string | number): Promise<RedmineTimeEntry | null> {
		if (!Settings.redmineApiKey || !Settings.redmineUserId || !issueId) return null;

		try {
			const response = await fetch(
				`${REDMINE_BASE_URL}/time_entries.json?issue_id=${issueId}&user_id=${Settings.redmineUserId}&sort=created_on:desc`,
				{
					headers: {
						'X-Redmine-API-Key': Settings.redmineApiKey,
					},
