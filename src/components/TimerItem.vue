...
const updateTime = () => {
  openModal(UpdateTime, {
    ...
    timerData: { time: props.timerData.time },
  });
};
const favorite = () => {
  try {
...
const interactTimer = (event: Event) => {
  ...
  if (props.isActive) TimerSystem.pauseActiveTimer();
  else TimerSystem.startTimer(props.timerId);
};

const showOnHover = computed(() => (Settings.hideOptions ? "hover-hide" : ""));
const hideLog = computed(() =>
  TimerSystem.isLoggable(props.timerData) ? "" : "hide"
);
const chevron = computed(() =>
  props.timerData.controlsHidden ? "fa-chevron-down" : "fa-chevron-up"
);
const showExtraControls = computed(() =>
  props.timerData.controlsHidden ? "d-none" : ""
);
const activeBgColor = computed(() =>
  props.isActive ? "bg-alt-active" : "bg-alt-default"
);
const issueLink = computed(
  () => `https://pm.mieweb.com/issues/${props.timerData.issue}`
);
</script>
<template>
  <div
    ...
    @click="interactTimer"
  >
    <div ref="timerGrid" class="timer-grid">
      <i
      ...
      <p ref="time" class="time m-0">
        {{ timerData.time }}
      </p>
      <div :class="`timer-options d-flex ${showOnHover}`">
        <i class="fa fa-edit timer-button" @click="edit" />
        <i class="fa fa-undo timer-button" @click="reset" />
        <i class="fa fa-plus timer-button" @click="updateTime" />
        <i
          ref="favoriteStar"
          class="fa fa-star timer-button"
          @click="favorite"
        />
        <i class="fa fa-trash-alt timer-button" @click="remove" />
      </div>
      <i
      ...
      <i class="fa fa-grip-vertical handle" />
    </div>
  </div>
</template>
<script lang="ts">
