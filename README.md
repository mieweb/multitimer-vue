...
The *Zeus QA* version is the official location of the MultiTimer, featuring the most stable version of the application. The *Zeus* version acts as a mirror, and should effectively be the same as *Zeus QA*. *Zeus Testing* is used for my test deployments in case the dev environment gives me a different experience than when it gets deployed. As a note, your browser's `localstorage` is the same on both *Zeus Testing* and *Zeus*, so you'll see the same timers and other data carried over between the two.
...
### Building the Timer
If you deem it necessary, you can run `npm run build` to build to the `dist/` directory. Useful if you want to push the contents of this directory to a server to see how it serves the contents up.
...
### "Importing Outlook meetings isn't working!"
